<?php

use Illuminate\Http\Request;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

// Route::middleware('auth:nv')->get('/user', function (Request $request) {
//     return $request->user();
// });

/**
 * http://localhost:9000/thanhvien/
 */
Route::group(['prefix' => 'web', 'middleware' => ['web','auth']], function () {
    /**
     * Route get all thanh_vien
     */
    Route::get('/', [
        'as'=>'nguyenlieu',
        'uses' =>'NguyenlieuController@index'
    ]);
    /**
     * Route get a thanh_vien
     */
    Route::get('/create', [
        'as'=>'createNl',
        'uses' =>'NguyenlieuController@create'
    ]);
    Route::get('/{id}', 'NguyenlieuController@show');
    /**
     * Route create new thanh_vien
     */
    Route::post('/create', [
        'as'=>'themNl',
        'uses' =>'NguyenlieuController@store'
    ]);
    // Route::post('/create', 'NguyenlieuController@store');
    /**
     * Route update a thanh_vien
     */
    Route::get('/update/{id}/edit', [
        'as'=>'updateNl',
        'uses' =>'NguyenlieuController@edit'
    ]);
    Route::post('/update/{id}',  [
        'as'=>'suaNl',
        'uses' =>'NguyenlieuController@update'
    ]);
    /**
     * Route delete a thanh_vien
     */
    Route::post('/remove/{id}', [
        'as'=>'xoaNl',
        'uses' =>'NguyenlieuController@destroy'
    ]);
});

